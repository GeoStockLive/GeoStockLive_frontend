  ...
  return (
    <aside className="sidebar h-screen shadow-panel">
      ...
      <nav className="flex flex-col gap-1 flex-1 w-full items-center">
        {nav.map(item => (
          <Link key={item.href} href={item.href} title={item.label}
            className={`nav-icon flex-col gap-0.5 h-12 ${path === item.href ? 'active' : ''}`}>
            {item.icon}
            <span className="text-[7px] tracking-widest font-mono">{item.label}</span>
          </Link>
        ))}
      </nav>
      ...
      <div className="flex flex-col gap-1 mt-auto">
        <button className="nav-icon" title="Settings"><CogIcon /></button>
        <button className="nav-icon" title="Help"><HelpIcon /></button>
      </div>
    </aside>
  );
}
...
function SignalIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 6l6 6-6 6"/><path d="M9 12h14"/></svg>;
}
function ChartIcon() {
...
function ClockIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg>;
}
function CogIcon() {
