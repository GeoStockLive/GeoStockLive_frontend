...
const IntelligencePanel = ({ selectedCountry }: IntelligencePanelProps) => {
  ...
  useEffect(() => {
    console.info(`[INTEL] Received country prop: ${selectedCountry}`);
  }, [selectedCountry]);
  ...
  useEffect(() => {
    ...
    const fetchIntel = async () => {
      setLoading(true);
      console.info(`[INTEL] Fetching for: ${selectedCountry}`);
      try {
        const apiBase = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
        const res = await fetch(`${apiBase}/risk/country/${encodeURIComponent(selectedCountry)}`);
        const data = await res.json();
        console.info(`[INTEL] Fetch successful for: ${selectedCountry}`, data);
        setIntel(data);
      } catch (err) {
        console.error(`[INTEL] Failed to fetch data for ${selectedCountry}:`, err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // FIX: Once a selection is made, we MUST stay out of standby mode.
  // We only show standby if selectedCountry is explicitly null/empty AND we aren't currently loading.
  const isSelectionActive = selectedCountry && selectedCountry.trim() !== "";
  
  if (!isSelectionActive && !loading) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center opacity-40">
        <div className="w-12 h-12 border border-gold-primary/20 rounded-full flex items-center justify-center mb-4">
        ...
        <div className="font-space font-bold text-xs uppercase tracking-widest text-gold-primary mb-2 text-glow-cyan">Global Sentinel Standby</div>
  ...
  // Display name during loading or error to avoid empty titles
  const displayName = selectedCountry || intel?.country || "Analyzing...";
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-y-auto scrollbar-thin p-5 gap-6 animate-slide-in-right">
      {/* Header */}
      <div>
      ...
      <div className="flex flex-col gap-3">
        <div className="card-high border-l-2 p-4 bg-background-elevated/50 group" style={{ borderColor: color }}>
          ...
          <div className="progress-track mt-2 bg-background-primary/40 h-1 overflow-hidden">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
             {intel?.events?.[0]?.assets?.map((asset, idx) => {
                ...
                return (
                  <div key={`${asset}-${idx}`} className="p-2.5 bg-background-primary/40 border border-white/5 rounded-sm hover:border-gold-primary/30 transition-colors">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center px-1 border-b border-white/5 pb-2">
           ...
           <span className="text-[8px] font-mono text-gold-primary/60 animate-signal">SYNCING</span>
        </div>

        <div className="flex flex-col gap-4">
          {intel?.events?.map((e, idx) => (
            <div key={e.id || idx} className="relative pl-3 flex flex-col gap-2">
              ...
              <div className="flex items-center justify-between mt-1 pt-2 border-t border-white/5">
                ...
                <span className="text-[8px] font-mono font-bold text-gold-primary uppercase tracking-widest">
          ...
          {(!intel?.events || intel.events.length === 0) && (
            <div className="py-10 text-center opacity-20">
               ...
               <div className="w-1 h-1 bg-gold-primary mx-auto rounded-full animate-ping" />
            </div>
          )}
        </div>
      </div>
